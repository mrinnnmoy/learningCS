...
contract LiquidityPool is ERC20 {
    ...
    // adding alongside the existing errors:
    error Expired();
    ...
    function swap(
        ...
        if (block.timestamp > deadline) revert Expired(); // Concept 10

        bool zeroForOne = amount0In > 0;
        (
            ...
            uint256 amountIn
                ...
                ? (token0, token1, reserve0, reserve1, amount0In)
                : (token1, token0, reserve1, reserve0, amount1In);

        tokenIn.transferFrom(msg.sender, address(this), amountIn);

        uint256 amountInWithFee = amountIn * 997; // Concept 4 — 0.3% fee
        amountOut =
            (amountInWithFee * reserveOut) /
            (reserveIn * 1000 + amountInWithFee);

        if (amountOut == 0) revert InsufficientOutputAmount();
        if (amountOut < minAmountOut) revert SlippageExceeded(); // Concept 10
